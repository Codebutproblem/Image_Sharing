...
function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!sent) {
      ...
      if (result.message === ResponseMessage.SEND_OTP_SUCCESS) {
        setSent(true);
        dispatch(
          showAlert({ type: "success", message: "Mã xác thực đã được gửi" }),
        );
      } else {
      ...
      if (/^\d+$/.test(otp) === false || otp.length !== 6) {
        dispatch(
          showAlert({ type: "error", message: "Mã xác thực không hợp lệ" }),
        );
        return;
      }
      ...
      if (result.message === "verify-otp-success") {
        ...
        dispatch(
          showAlert({ type: "error", message: "Mã xác thực không đúng" }),
        );
      }
    }
  };
  ...
  return (
    <div className="px-6 w-[500px] max-w-[500px] min-w-[300px] border rounded-xl border-slate-400 p-5 text-slate-200 min-h-[300px] backdrop-blur-lg">
      <div className="text-center font-semibold text-4xl mb-6">
        Xác thực tài khoản
      </div>
      ...
      <div className="text-center">
        <Link
          ...
          className="inline-block p-3 border border-slate-400 rounded-full duration-200 hover:text-black hover:bg-slate-50"
        >
