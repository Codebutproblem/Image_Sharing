...
function CommentBox({ setReload, reload }) {
  ...
  const handleDelete = async (e) => {
    ...
    if (result.message === ResponseMessage.DELETE_SUCCESS) {
      dispatch(
        showAlert({ type: "success", message: "Xóa bình luận thành công" }),
      );
      setReload(reload + 1);
    }
  };
    ...
    <>
      <div className="w-ful mb-8">
        {commentObject.comments.map((comment) => (
          <div
            key={comment.id}
            className="flex items-center gap-3 p-3 pb-0 mb-4"
          >
            <Link to={`/profile/${comment.Author.slug}`}>
              <img
                src={
                  comment.Author.avatar || "https://www.gravatar.com/avatar/"
                }
              ...
              <div className="flex gap-3 items-end">
                ...
                {userId === comment.Author.id && (
                  <button
                    ...
                    className="text-sm font-medium bg-gray-400 duration-200 hover:bg-gray-500 text-slate-50 px-1 py-0.5 rounded-md"
                  >
                    Xóa
                  </button>
                )}
