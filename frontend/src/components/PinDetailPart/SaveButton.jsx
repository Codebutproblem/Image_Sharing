...
function SaveButton({ pin }) {
  ...
  if ((pin.Tables && pin.Tables.find((table) => table.user_id === user.id) || saved)){
    return (
      <button onClick={handleUnSave} className=" text-xs sm:text-base p-2 bg-gray-500 rounded-xl text-slate-50">
        Đã lưu
      </button>
    );
  };
  return (
    <div>
      ...
      {showSaveBox && <SavePinBox setShowSaveBox={setShowSaveBox} setSaved={setSaved} pinId={pin.id} />}
    </div>
